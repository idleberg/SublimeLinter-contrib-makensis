...
------------|---------|------------
`ppo_mode`  | `true`  | Lints much faster, but ignores warning (unless in strict mode). Equivalent of `-PPO` flag.
`safe_mode` | `false` | Treats warnings as errors, requires NSIS v3.0 (or higher). Equivalent of `-WX` flag.
